...
class ProcessEndingBalanceController extends Controller
{   
    public function processEndBalSina($m_date, $y_date, $acc_no, $acc_no_end)
    {        
        ...
        $insertData = []; // Initialize insertData array here
        ...
        // --- Start of new code for insertion ---

        // 1. Clear existing data for the current period and division (optional, but recommended for temp tables)
        // This prevents duplicate entries if the report is run multiple times for the same period.
        DB::table('temp_tb_ending_balance')
            ->where('code_periode', $code_period)
            ->delete();

        // 2. Insert the combined data into temp_tb_ending_balance
        if (!empty($insertData)) {
            // Use insert method for multiple rows for efficiency
            DB::table('temp_tb_ending_balance')->insert($insertData);
        }

        // --- End of new code for insertion ---
